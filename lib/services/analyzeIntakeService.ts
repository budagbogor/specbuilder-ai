...
import { type IntakeSchemaValues } from "@/lib/validators/intakeSchema";
...
export async function analyzeIntake(intake: IntakeSchemaValues): Promise<AnalyzeIntakeResult> {
  const fallbackData = buildAnalyzeFallback(intake);
  const sanitizedIntake = sanitizePromptInput(intake);
  const prompt = buildAnalyzeIntakePrompt(sanitizedIntake);
